...
def detect_faces(net, frame):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.7:  # Confidence threshold
            ...
            # Add extra size around the detected face
            extra_size = 50
            x -= extra_size
            y -= extra_size
            x1 += extra_size
...
def process_frame(net, frame_queue, output_queue):
    ...
    while True:
        ...
        for idx, (x, y, w, h) in enumerate(face_locations):
            ...
            # Save the detected face only once every 5 seconds
            if (datetime.datetime.now() - last_save_time).total_seconds() >= 300:
...
def generate_frames():
    ...
    while True:
        ...
        if not success:
            # If frame reading fails, attempt to reconnect
